#!/usr/bin/env bash
# =============================================================================
#  uninstall.sh — Désinstallation de PostgreSQL + pgvector
#  Propose deux modes :
#    soft  : supprime les ressources K8s — données conservées dans /data/postgresql
#            → réinstallation possible avec ./install.sh sans perte de données
#    hard  : supprime tout + efface /data/postgresql du disque
#            → IRRÉVERSIBLE, toutes les données sont perdues
# =============================================================================
...
echo ""
echo "════════════════════════════════════════════════════"
echo "   PostgreSQL + pgvector — Désinstallation"
echo "════════════════════════════════════════════════════"
echo ""
echo "  Choisir le mode :"
echo "  [1] Soft  — supprime les ressources K8s, données conservées dans ${DATA_DIR}"
echo "              → réinstallation possible avec ./install.sh sans perte de données"
echo "  [2] Hard  — supprime tout + efface ${DATA_DIR} du disque"
echo "              → IRRÉVERSIBLE, toutes les données sont perdues"
echo "  [q] Annuler"
echo ""
...
# ─── 1. StatefulSet ───────────────────────────────────────────────────────────
info "Suppression du StatefulSet postgresql..."
kubectl delete statefulset postgresql -n "$NAMESPACE" --ignore-not-found 2>/dev/null && \
    success "StatefulSet supprimé." || warn "StatefulSet introuvable."

# Attendre que le pod disparaisse
if kubectl get pods -n "$NAMESPACE" -l app=postgresql --no-headers 2>/dev/null | grep -q .; then
    info "Attente de la terminaison du pod..."
    kubectl wait --for=delete pod -l app=postgresql -n "$NAMESPACE" --timeout=60s 2>/dev/null || \
        kubectl delete pod -l app=postgresql -n "$NAMESPACE" --force --grace-period=0 2>/dev/null || true
fi
success "Pod terminé."

# ─── 2. Services ──────────────────────────────────────────────────────────────
info "Suppression des Services..."
kubectl delete service postgresql postgresql-nodeport -n "$NAMESPACE" --ignore-not-found 2>/dev/null
...
info "Suppression du ConfigMap postgres-init-sql..."
kubectl delete configmap postgres-init-sql -n "$NAMESPACE" --ignore-not-found 2>/dev/null
...
kubectl delete -f "${SCRIPT_DIR}/pv-postgresql.yaml" --ignore-not-found 2>/dev/null || true
success "PV supprimé."
