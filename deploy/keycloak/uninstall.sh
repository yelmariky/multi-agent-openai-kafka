#!/usr/bin/env bash
# =============================================================================
#  uninstall.sh — Désinstallation de Keycloak
#  Propose deux modes :
...
#    hard  : supprime tout + efface /Users/younes/data/keycloak du disque
...
NAMESPACE="keycloak"
DATA_DIR="/Users/younes/data/keycloak"
...
# ─── 1. Deployment ────────────────────────────────────────────────────────────
info "Suppression du Deployment keycloak..."
kubectl delete deployment keycloak -n "$NAMESPACE" --ignore-not-found 2>/dev/null && \
    success "Deployment supprimé." || warn "Deployment introuvable."

# Attendre que le pod disparaisse
if kubectl get pods -n "$NAMESPACE" -l app=keycloak --no-headers 2>/dev/null | grep -q .; then
    info "Attente de la terminaison du pod..."
    kubectl wait --for=delete pod -l app=keycloak -n "$NAMESPACE" --timeout=60s 2>/dev/null || \
        kubectl delete pod -l app=keycloak -n "$NAMESPACE" --force --grace-period=0 2>/dev/null || true
fi
success "Pod terminé."

# ─── 2. Services ──────────────────────────────────────────────────────────────
info "Suppression des Services..."
kubectl delete service keycloak keycloak-nodeport -n "$NAMESPACE" --ignore-not-found 2>/dev/null
success "Services supprimés."

# ─── 3. Secret ────────────────────────────────────────────────────────────────
info "Suppression du Secret keycloak-secret..."
kubectl delete secret keycloak-secret -n "$NAMESPACE" --ignore-not-found 2>/dev/null
success "Secret supprimé."

# ─── 4. ConfigMap realm ───────────────────────────────────────────────────────
info "Suppression du ConfigMap keycloak-realm-config..."
kubectl delete configmap keycloak-realm-config -n "$NAMESPACE" --ignore-not-found 2>/dev/null
success "ConfigMap supprimé."

# ─── 5. PVC ───────────────────────────────────────────────────────────────────
info "Suppression du PersistentVolumeClaim keycloak-pvc..."
kubectl delete pvc keycloak-pvc -n "$NAMESPACE" --ignore-not-found 2>/dev/null
success "PVC supprimé."

# ─── 6. PV ────────────────────────────────────────────────────────────────────
info "Suppression du PersistentVolume keycloak-pv..."
kubectl delete pv keycloak-pv --ignore-not-found 2>/dev/null
kubectl delete -f "${SCRIPT_DIR}/pv-keycloak.yaml" --ignore-not-found 2>/dev/null || true
success "PV supprimé."

# ─── 7. StorageClass ──────────────────────────────────────────────────────────
info "Suppression de la StorageClass keycloak-hostpath..."
kubectl delete storageclass keycloak-hostpath --ignore-not-found 2>/dev/null
