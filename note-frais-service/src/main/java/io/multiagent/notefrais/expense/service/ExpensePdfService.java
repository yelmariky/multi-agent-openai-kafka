...
public class ExpensePdfService {
    ...
    public byte[] buildPdf(String start, String end, String type, String currency, String company) {
        return buildPdf(start, end, type, currency, company, null);
    }

    public byte[] buildPdf(String start, String end, String type, String currency, String company, String consultantEmail) {
        ExpenseReportResponse report = expenseReportService.report(start, end, type, currency, company, consultantEmail);
        ...
        try (PDDocument doc = new PDDocument()) {
            ...
            try {
                ...
                writeLine(cs, margin, y, "Total dep. personnelles (hors km et location) : " + safeMoney(totalPersonnelHorsKmLoc));
                y -= leading;
                writeLine(cs, margin, y, "Total dep. business (hors km et location) : " + safeMoney(totalBusinessHorsKmLoc));
                y -= leading;
                writeLine(cs, margin, y, "Total location (domiciliation) : " + safeMoney(totalLocation));
                y -= leading;
                writeLine(cs, margin, y, "Km (dates de depense, lun-ven) : " + safeMoney(totalKm));
                y -= leading;
                writeLine(cs, margin, y, "Cout km : " + safeMoney(costKm));
                y -= leading;
                writeLine(cs, margin, y, "Total general (perso + business + location + km) : " + safeMoney(totalGeneral));
                y -= leading * 2;
                cs.setFont(PDType1Font.HELVETICA_BOLD, 11);
                writeLine(cs, margin, y, "Total a rembourser (Personnel, incluant location & km) : " + safeMoney(totalRembPersonnel));
            } finally {
                if (cs != null) {
                    cs.close();
                }
            ...
            throw new IllegalStateException("Echec generation PDF : " + e.getMessage(), e);
        }
    }
    ...
    private float drawExpensePageHeader(
        ...
        y -= leading;
        if (report.getConsultantEmail() != null && !report.getConsultantEmail().isBlank()) {
