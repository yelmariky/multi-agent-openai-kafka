...
public class ExpenseExcelService {
    ...
    public byte[] buildExcel(ExpenseReportResponse report) {
        ...
        try (Workbook wb = new XSSFWorkbook()) {
            ...
            if (report.getConsultantEmail() != null && !report.getConsultantEmail().isBlank()) {
                Row consultantRow = detail.createRow(nextRow);
                consultantRow.createCell(0).setCellValue("Consultant : " + report.getConsultantEmail());
                consultantRow.getCell(0).setCellStyle(header);
            ...
            DataValidation payValidation = dvHelper.createValidation(payConstraint, new CellRangeAddressList(nextRow, 500, 5, 5));
    ...
    private List<java.time.LocalDate> computeKmDates(ExpenseReportResponse report) {
        ...
        if (report.getExpenses() != null) {
            for (ExpenseItem e : report.getExpenses()) {
