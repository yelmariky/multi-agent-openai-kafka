package io.multiagent.notefrais.weaviate;
...
/**
 * Utilitaires pour extraire des champs simples à partir d'une réponse GraphQL Weaviate.
 */
@Slf4j
public final class WeaviateResponseParser {
    ...
    private static ExpenseItem mapToExpenseItem(Map<?, ?> map) {
        ...
        if (amount != null) {
            try { item.setAmount(Double.parseDouble(amount.toString())); } catch (Exception ignored) { }
        }
        ...
        if (absenceJson != null && !absenceJson.toString().isBlank()) {
            try {
                ...
                log.warn("Could not parse absencePeriodsJson: {}", e.getMessage());
            }
        }
        return item;
    }
