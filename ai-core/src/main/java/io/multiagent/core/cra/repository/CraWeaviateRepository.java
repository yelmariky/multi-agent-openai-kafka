...
import static io.multiagent.core.weaviate.WeaviateUtils.isNullOrBlank;
...
public class CraWeaviateRepository {
        ...
        try {
            ...
            if (cra.entries() != null && !cra.entries().isEmpty()) {
                try {
                    ...
                    log.warn("⚠️ indexCra: could not serialize entries: {}", e.getMessage());
                }
            }
        ...
        try {
            ...
            for (var obj : response.getResult()) {
                ...
                if (!isNullOrBlank(consultant)) {
                    String name = safeString(props.get("consultant")).toLowerCase(java.util.Locale.ROOT);
                    if (!name.contains(consultant.toLowerCase(java.util.Locale.ROOT))) continue;
                }
                ...
                if (!entriesJson.isBlank()) {
                    try {
                        ...
                        log.warn("⚠️ findCrasByPeriod: could not parse entriesJson: {}", e.getMessage());
                    }
                }
    ...
    public void deleteCra(String id) {
        try {
            if (isNullOrBlank(id)) {
                log.warn("⚠️ deleteCra: id is blank, skipping");
                return;
            }
            ...
            if (result.hasErrors()) {
                ...
                log.info("🗑️ Weaviate: CRA supprimé (id={})", id);
            }
        } catch (Exception e) {
