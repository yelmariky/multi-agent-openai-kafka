...
import static io.multiagent.core.weaviate.WeaviateUtils.*;
...
public class ExpenseWeaviateRepository {
        ...
        try {
            ...
            if (item.getAbsencePeriods() != null && !item.getAbsencePeriods().isEmpty()) {
                try {
                    ...
                    log.warn("⚠️ Could not serialize absencePeriods: {}", ex.getMessage());
                }
            }
    ...
    public List<ExpenseItem> findExpensesBetween(LocalDate start, LocalDate end, String type, String currency, String consultantEmail) {
        try {
            ...
            log.debug("⚠️ Weaviate date query returned empty; trying fallback on dateText (string)");
            ...
            for (var obj : response.getResult()) {
                ...
                if (!absenceJson.isBlank()) {
                    try {
                        ...
                        log.warn("⚠️ findKmExpenseAbsences: could not parse absencePeriodsJson: {}", e.getMessage());
                    }
                }
            }
            return result;
